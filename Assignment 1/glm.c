...
glmInitVBO(GLMmodel* model)
{
    ...
    GLfloat* vertices = malloc(sizeof(GLfloat) * model->numtriangles * 3 * 3);
    GLfloat* normals = malloc(sizeof(GLfloat) * model->numtriangles * 3 * 3);
    ...
    glGenBuffersARB(1, &verticesID);                  // Get A Valid Name
    glBindBufferARB(GL_ARRAY_BUFFER_ARB, verticesID);         // Bind The Buffer
    // Load The Data
    glBufferDataARB(GL_ARRAY_BUFFER_ARB, model->numtriangles*sizeof(GLfloat)*3*3, vertices, GL_STATIC_DRAW_ARB);
        
    glGenBuffersARB(1, &normalsID);                  // Get A Valid Name
    glBindBufferARB(GL_ARRAY_BUFFER_ARB, normalsID);         // Bind The Buffer
    // Load The Data
    glBufferDataARB(GL_ARRAY_BUFFER_ARB, model->numtriangles*sizeof(GLfloat)*3*3, normals, GL_STATIC_DRAW_ARB);
    free(vertices);
    free(normals);
}
